...
| `api` | `rcab/api:<ref>` | Node.js process. Depends on `postgres`, `redis`, `osrm`, `migrator` |
...
| `alertmanager` | `prom/alertmanager` | Alert routing (Slack in prod) |
| `uptime-kuma` | `louislam/uptime-kuma` | Synthetic uptime probes |
...
- `pg-data`, `redis-data`, `osrm-data`, `loki-data`, `prom-data`, `grafana-data`, `uptime-kuma-data` — named volumes.
