...
| `/ride/:id` | Active solo ride — single status action button (Start trip → I've arrived → Start ride → End ride) + prominent "Navigate" → Google Maps | RCAB-E4.S6. Each tap `POST`s `/v1/rides/:id/state`; status hydrated via `GET /v1/rides/:id` on mount (reconnect-restore). "Navigate" targets the pickup until `in_progress`, then the dropoff. Auto-routes to `/rating/:id` on `completed`. |
